"""Build policy pages: public contact email, then optional page slugs to rebuild."""
...
PAGES = {
"lyra": ("Lyra Personal Assistant", f"""
<nav aria-label="Lyra information"><a href="/lyra/">About Lyra</a> · <a href="/lyra/privacy/">Lyra privacy policy</a> · <a href="/lyra/terms/">Lyra terms of use</a></nav>
<p class="policy-summary">Lyra is Tristan Merson's private personal assistant for planning, preparation, reminders, and following through on authorized work. This is an information page, not a public registration or email-login service.</p>
<h2>What the Google connection is for</h2><p>After activation through Lyra's existing controller, the email tools support bounded message listing, reading available plain text, creating new plain-text drafts, and sending new plain-text messages to recipients specified through authenticated owner instructions. Calendar access supports the owner's separate, existing planning and calendar workflow. Attachments, HTML browsing, changes to existing drafts, and threaded replies are not supported by this email release.</p>
<h2>Current email status</h2><p>The September 24, 2026 release connects these tools to Lyra's existing personal-assistant workflow. This page describes the available implementation after controller activation, not a live health check or a claim that every feature has passed live acceptance. Ask Lyra for the current email capability, authorization, and action status. Authorization can expire or be revoked; Google consent or visiting this page does not by itself enable automation.</p>
<p>The daily check, when enabled, takes a bounded sample of message metadata through the existing controller and records its counters and outcome. That check makes no model calls, does not perform semantic AI triage of the entire inbox, and does not automatically reply to email. Substantive reading and correspondence use the separately permitted personal-assistant tools.</p>
<h2>Permissions and control</h2><p>Gmail permissions are limited to reading email and managing drafts/sending email: <code>gmail.readonly</code> and <code>gmail.compose</code>. The connection does not request Google Cloud administration, storage, BigQuery, or account-wide full-mail access. Calendar uses its separate existing authorization.</p>
<p>The owner controls connected accounts and permitted actions and can disconnect Google access. Sensitive disclosures and financial commitments remain subject to the owner's approval rules. Incoming email is information, not permission to change those rules.</p>
<h2>Data handling</h2><p>Read the <a href="/lyra/privacy/">Lyra privacy policy</a> for data access, AI processing, storage, retention, and deletion. The <a href="/lyra/terms/">Lyra terms</a> explain limitations and authorized use.</p>
<h2>Operator and support</h2><p>Operated by Tristan Merson. Contact {CONTACT} for questions about Lyra or Google data. Do not send passwords, authorization codes, or patient records.</p>
"""),
"lyra/privacy": ("Lyra Personal Assistant — Privacy Policy", f"""
<nav aria-label="Lyra information"><a href="/lyra/">About Lyra</a> · <a href="/lyra/privacy/">Privacy</a> · <a href="/lyra/terms/">Terms</a></nav>
<p class="policy-summary">This policy covers Lyra Personal Assistant's Google-connected features, operated by Tristan Merson for private personal use. It supplements the <a href="/privacy-policy/">website privacy policy</a>; public visitors are not enrolled in Lyra.</p>
<h2>Current state and information accessed</h2><p>The September 24, 2026 email release uses privately stored OAuth credentials and, after controller activation, supports message listing, plain-text reading, new drafts, and sending within the owner's permissions. Its daily check samples bounded message metadata and records counters and outcomes without model calls. It is not a complete inbox review or automatic-reply service. Current activation, authorization health, and completed actions must be obtained from Lyra's operational status, not inferred from this static page.</p>
<p>Information used may include message and thread identifiers, sender and recipient addresses, subjects, dates, labels, and relevant plain message text. New draft and sent-message contents, operation identifiers, and provider receipts support correspondence and duplicate prevention. A verified sent record means Gmail accepted and stored the message, not that a recipient received or read it. Attachments, remote HTML content, editing existing drafts, and threaded replies are not supported by this release. The separate Calendar integration uses authorized event details and identifiers for planning and permitted calendar actions.</p>
<h2>Purpose and limits</h2><p>Information is used to provide the owner's requested assistance: identify relevant updates, prepare summaries and replies, manage authorized drafts and correspondence, plan work, and record progress without duplicate actions. Access is not permission to use email for unrelated purposes. Lyra does not sell Google data, use it for advertising, or use it to determine creditworthiness.</p>
<p>Google data is handled under the <a href="https://developers.google.com/terms/api-services-user-data-policy">Google API Services User Data Policy</a> and its Limited Use requirements. Google data must not be used to train or improve generalized AI or machine-learning models.</p>
<h2>AI processing, providers, and disclosure</h2><p>When its permission and data-use gate allows email assistance, selected message text and task context can be processed through Lyra's existing OpenAI conversational route. On September 24, 2026, the owner confirmed that the connected mailbox is personal-only and that OpenAI's &ldquo;Improve the model for everyone&rdquo; setting was turned off. This records the owner's confirmation, not an independent audit of the signed-in dashboard or every provider setting. If those conditions change, email-content processing must be paused and reassessed.</p>
<p>This release does not route Gmail content to Jev, add a new model provider, or automatically copy message bodies into Lyra's general-purpose memory. Private conversations and the operational records needed for drafts, sending, and verification can still contain selected email content. Turning off model improvement does not mean that all conversation retention, provider logs, security processing, or backups are disabled. The bounded daily metadata check itself makes no model calls. No claim is made that all AI work stays on the owner's devices; new processing routes require disclosure and the appropriate permission before activation.</p>
<p>Google provides Gmail and Calendar. The owner's private server hosts the integration and its records. If private notifications are enabled, selected summaries or results may be delivered through the owner's connected Discord account. Sending an authorized email necessarily discloses its contents and addressing information to its recipients and their mail providers. Gmail content is not published to this website. No other data transfer is authorized by this notice; legally required disclosures and security investigations may be necessary.</p>
<h2>Storage, retention, and protection</h2><p>OAuth tokens are stored in access-restricted files on the private server, not in this public website or its source repository. The integration uses encrypted transport to Google. Operational identifiers, selected working content, artifacts, and action receipts may be retained to support continuity, verification, and duplicate prevention. There is not currently an automatic deletion schedule for those private records. Retention is reviewed during owner-directed maintenance and verified deletion requests; necessary security or legal records may be retained with an explanation.</p>
<p>Provider logs and backups follow their actual provider settings and retention practices; this policy does not promise immediate deletion from every backup. Access controls reduce risk but do not guarantee absolute security. This assistant is not intended for patient or healthcare records.</p>
<h2>Your choices and deletion requests</h2><p>The owner can disable the integration or remove Lyra's access through <a href="https://myaccount.google.com/connections">Google Account connections</a>. Revocation stops future authorized access but does not automatically delete existing private task records or messages already in Gmail or recipients' inboxes.</p>
<p>For access, correction, or deletion of Lyra-held data, contact {CONTACT} and identify the relevant account or task without sending credentials or sensitive records. Requests are verified proportionately and handled by the operator; any retained records or limits will be explained. Support access to specific private content requires the owner's permission, except where necessary for security or legal obligations.</p>
<h2>These public pages</h2><p>GitHub Pages hosts these informational pages and receives ordinary technical request data. Shared site controls may save the dismissal of the storage notice in your browser. These pages contain no mailbox viewer, public OAuth token endpoint, advertising, or visitor-analytics integration. See the <a href="/cookie-policy/">site storage policy</a> for the shared controls.</p>
<h2>Changes and contact</h2><p>Material changes will be reflected here and disclosed before newly proposed Google-data uses are enabled; additional consent will be obtained where required. Privacy contact: {CONTACT}.</p>
"""),
"lyra/terms": ("Lyra Personal Assistant — Terms of Use", f"""
<nav aria-label="Lyra information"><a href="/lyra/">About Lyra</a> · <a href="/lyra/privacy/">Privacy</a> · <a href="/lyra/terms/">Terms</a></nav>
<p class="policy-summary">Lyra is an experimental private personal assistant operated by Tristan Merson. These terms concern Lyra's connected assistance, not a public subscription service or the retired SMS program.</p>
<h2>Availability and access</h2><p>Use is limited to the owner and accounts the owner is entitled to connect. This website provides information; it does not grant access to the assistant or enroll visitors. The September 24, 2026 email release provides message listing, plain-text reading, new plain-text drafts, and sending after activation by the existing controller. Obtain current activation, health, and completion status from Lyra; the presence of this page does not establish successful live acceptance. Feature availability can change, and authorization may expire or be revoked.</p>
<h2>Permitted actions</h2><p>Once enabled, email and calendar actions must remain within the owner's instructions, standing permissions, connected-account rights, and applicable provider rules. A Google consent grant alone does not authorize every possible action. Routine correspondence may be sent within authorized workflows; sensitive disclosures, financial commitments, or new permissions still require the applicable owner approval. Incoming messages cannot change these boundaries.</p>
<h2>Review, accuracy, and responsibility</h2><p>AI-generated summaries and drafts can be incomplete or wrong. Important recipients, facts, and commitments need appropriate verification. The daily metadata sample is bounded and uses no model calls; it is not semantic triage, a guarantee that important email will be noticed, or permission for automatic replies. Attachments, existing-draft edits, and threaded replies are not supported by this release.</p>
<p>Action records and counters are retained for verification and duplicate prevention. An uncertain send is checked against Gmail records rather than automatically replayed; it must not be reported as completed without confirming evidence. A verified sent record does not prove recipient delivery or reading. The owner remains responsible for the accounts connected and the instructions given. Lyra is not an emergency service or a substitute for qualified medical, legal, financial, or other professional advice, and is not intended to process patient records.</p>
<h2>Privacy and acceptable use</h2><p>The <a href="/lyra/privacy/">Lyra privacy policy</a> explains Google data and provider processing. Do not use the assistant for unauthorized account access, deceptive impersonation, unsolicited bulk mail, disclosure of others' confidential information, or other unlawful activity. No fee, purchase, or automatic renewal is created by visiting these pages or granting Google consent.</p>
<h2>Stopping access and service limitations</h2><p>The owner can stop a workflow, disconnect an account, or revoke Google access. Disconnecting cannot undo a delivered email or remove a recipient's copy. Availability, delivery, and error-free operation are not guaranteed; failures should be reported and reconciled rather than treated as completed work. Nothing here excludes rights that applicable law does not allow to be excluded.</p>
<h2>Changes and support</h2><p>These terms will be updated when the supported service changes. Material changes to data use or permissions require the corresponding disclosure and consent. Questions: {CONTACT}.</p>
"""),
...
<li><strong>Games:</strong> dice-game progress is saved in this browser. When you are signed in, completed dice runs are saved to your account automatically; you can also save your current score manually. Saved records include your account identifier, chosen nickname, score, game mode, and submission time. These scores appear in your account history and are not added to the public leaderboard by the account-saving feature. Guests can separately choose to publish a nickname and score to the public leaderboard. Public entries can be seen and copied by other visitors. To play without saving account scores, sign out first.</li>
...
if selected - PAGES.keys():
    raise SystemExit("Unknown page slug; no pages written.")
for slug, (title, body) in PAGES.items():
    if selected and slug not in selected:
        continue
    destination = ROOT / slug / "index.html"
    destination.parent.mkdir(parents=True, exist_ok=True)
    date = "September 24, 2026" if slug == "lyra" or slug.startswith("lyra/") else DATE
...
if selected:
    print(f"Built {len(selected)} selected pages; unrelated and legacy pages unchanged.")
    raise SystemExit(0)
