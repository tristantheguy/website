...
<h2>What the Google connection is for</h2><p>The Gmail integration is being prepared to review relevant email, summarize items needing attention, prepare useful replies as drafts, and send correspondence within the owner's instructions and standing permissions. Calendar access supports the owner's separate, existing planning and calendar workflow.</p>
<h2>Current email status</h2><p>As of September 24, 2026, Gmail authorization and a metadata-only connection check have succeeded. The daily email review, draft, and send workflow is not yet active. Authorizing Google access or visiting this page does not by itself enable email automation.</p>
...
<h2>Current state and information accessed</h2><p>As of September 24, 2026, the Gmail connection stores OAuth credentials and has performed a metadata-only connection check. Daily email reading, drafting, and sending are not yet enabled. No email message content has been processed by that workflow.</p>
<p>When the authorized email features are enabled, information used may include message and thread identifiers, sender and recipient addresses, subjects, dates, labels, and relevant message text; draft and sent-message contents and delivery records are used to carry out and verify correspondence. Attachment processing requires a separately enabled capability and is not currently active. The separate Calendar integration uses authorized event details and identifiers for planning and permitted calendar actions.</p>
...
<h2>AI processing, providers, and disclosure</h2><p>Substantive email assistance may require selected message text and task context to be processed by the owner's configured AI provider, currently OpenAI. That email-processing route is not active. Before activation, its applicable data-use settings and terms must be checked against the Google-data restrictions above; this notice and the owner's in-product disclosure must be updated if processing or providers change. No claim is made that all AI work stays on the owner's devices.</p>
